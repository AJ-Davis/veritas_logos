...
async def list_verification_chains_protected(
    request,
    current_user: User = Depends(get_current_user)
):
...
@limiter.limit("50/minute")
async def get_verification_chain_protected(
    request,
    chain_id: str,
    current_user: User = Depends(get_current_user)
):
...
async def submit_verification_task_protected(
    request,
...
async def get_task_status_protected(
    request,
    task_id: str,
    current_user: User = Depends(get_current_user)
):
...
@limiter.limit("30/minute")
async def get_task_result_protected(
    request,
    task_id: str,
    current_user: User = Depends(get_current_user)
):
...
@limiter.limit("30/minute")
async def list_tasks_protected(
    request,
    current_user: User = Depends(get_current_user)
):
...
async def cancel_task_protected(
    request,
    task_id: str,
    current_user: User = Depends(get_current_user)
):
