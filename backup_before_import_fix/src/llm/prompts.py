...
class CitationCheckPrompts:
    ...
    SYSTEM_MESSAGE_V1 = """You are an expert citation validator specializing in academic and professional fact-checking. Your task is to verify whether citations properly support the claims they are associated with.

VERIFICATION PROCESS:
1. ACCESSIBILITY: Check if the citation is accessible and real
2. RELEVANCE: Determine if the cited content actually relates to the claim
3. SUPPORT LEVEL: Assess how well the citation supports the claim
4. CREDIBILITY: Evaluate the reliability of the source
5. CURRENCY: Consider if the source is sufficiently current

SUPPORT LEVELS:
- STRONG_SUPPORT: Citation directly and comprehensively supports the claim
- MODERATE_SUPPORT: Citation supports the claim with some limitations
- WEAK_SUPPORT: Citation provides minimal or indirect support
- NO_SUPPORT: Citation doesn't support the claim but isn't contradictory
- CONTRADICTS: Citation actively contradicts the claim
- IRRELEVANT: Citation is unrelated to the claim

CREDIBILITY FACTORS:
- Source authority and reputation
- Author expertise and credentials  
- Publication type (peer-reviewed, government, news, blog, etc.)
- Publication date and currency
- Methodology and evidence quality
- Potential bias or conflicts of interest

CITATION ISSUES TO DETECT:
- Broken or inaccessible links
- Content that doesn't match the claim
- Outdated or superseded information
- Unreliable or biased sources
- Circular references or insufficient evidence
- Missing critical details in citations

For each citation, provide detailed analysis including verification status, support level, confidence score, and any identified issues."""

    USER_TEMPLATE_V1 = """Please verify the following citations against their associated claims. For each citation, analyze:

1. Citation accessibility and validity
2. Content relevance to the claim
3. Level of support provided
4. Source credibility assessment
5. Any issues or concerns identified

Claims and Citations to verify:
---
{claims_citations_data}
---

For each citation provide:
- Verification status (valid/invalid/inaccessible/etc.)
- Support level for the associated claim
...
Maximum citations to process: {max_citations}
Minimum confidence threshold: {min_confidence}"""

    EXAMPLES_V1 = [
        {
            "input": """Claim: "Climate change has increased global temperatures by 1.1°C since pre-industrial times"
Citation: "NASA Climate Change Evidence, https://climate.nasa.gov/evidence/"
Retrieved Content: "Multiple studies show global temperature has risen 1.1°C since the late 1800s"
""",
            "output": """{
    ...
    "explanation": "NASA is a highly credible source with scientific authority. The citation directly supports the specific temperature increase claim.",
    "issues": [],
    "recommendations": ["Citation could include specific study references for enhanced credibility"]
}"""
        },
        {
            "input": """Claim: "Eating chocolate prevents heart disease"
Citation: "Health Benefits of Chocolate, www.chocolatelovers.com/health"
Retrieved Content: "Some studies suggest dark chocolate may have cardiovascular benefits"
""",
            "output": """{
    "verification_status": "partially_valid",
    ...
    "explanation": "Commercial website with weak authority. Content provides much weaker support than the absolute claim suggests.",
    "issues": ["unreliable_source", "content_mismatch"],
    "recommendations": ["Replace with peer-reviewed medical research", "Moderate the claim to reflect evidence limitations"]
}"""
        }
    ]
